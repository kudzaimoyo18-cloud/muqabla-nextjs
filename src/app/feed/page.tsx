...
import {
  MapPin, DollarSign, Briefcase, Clock, ChevronUp, ChevronDown,
  Bookmark, BookmarkCheck, Send, Check, Building2, Loader2, Play, Volume2, VolumeX,
} from 'lucide-react';
...
export default function FeedPage() {
  ...
  const { user, profile, initialize, isAuthenticated } = useAuthStore();
  ...
  const handleSave = useCallback(async () => {
    if (!currentJob || !profile?.id) return;
    if (savedJobs.has(currentJob.id)) return;
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <div
        ref={containerRef}
        className="h-[calc(100vh-72px)] relative overflow-hidden"
        {...handlers}
      >
        {currentJob && (
          <div
            ...
            <div className="flex-1 relative bg-gradient-to-b from-gray-900 to-[#0a0a0a] overflow-hidden">
              ...
              <div className="absolute right-4 bottom-8 flex flex-col gap-4 z-10">
                ...
                <button
                  ...
                  className="w-12 h-12 bg-emerald-500 hover:bg-emerald-600 disabled:bg-emerald-500/50 rounded-full flex items-center justify-center transition-colors"
                >
              ...
              <div className="absolute left-4 top-1/2 -translate-y-1/2 hidden md:flex flex-col gap-2 z-10">
                <button
                  ...
                  className="w-10 h-10 bg-black/40 backdrop-blur-sm rounded-full flex items-center justify-center disabled:opacity-30"
                >
            ...
            <div className="px-5 py-5 bg-[#0a0a0a] border-t border-white/[0.06]">
              ...
              <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/[0.04] md:hidden">
                <span className="text-xs text-gray-600 flex items-center gap-1">
                  <span className="text-emerald-400">←</span> Swipe to apply
                </span>
                <span className="text-xs text-gray-600 flex items-center gap-1">
                  Swipe to save <span className="text-blue-400">→</span>
                </span>
              </div>
            </div>
                  ...
                  <div className="flex items-center gap-3">
                    {actionFeedback.type === 'applied' ? (
                      <>
                        ...
                        <span className="text-lg font-semibold text-emerald-400">Applied!</span>
                      </>
                    ) : (
                      <>
                        <BookmarkCheck className="w-6 h-6 text-blue-400" />
                        <span className="text-lg font-semibold text-blue-400">Saved!</span>
                      </>
                    )}
                  </div>
                </div>
